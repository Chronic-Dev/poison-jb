...
int boot_ramdisk() {
	...
	debug("Loading kernelcache\n");
	error = irecv_send_command(client, "go kernel load 0x41000000 0xF00000");
	if(error != IRECV_E_SUCCESS) {
		error("Unable to load kernelcache\n");
		return -1;
	}

	debug("Patching kernelcache\n");
	error = irecv_send_command(client, "go kernel patch");
	if(error != IRECV_E_SUCCESS) {
		error("Unable to patch kernelcache\n");
		return -1;
	}

	debug("Booting kernelcache\n");
...
int execute_ibss_payload() {
	...
	// If boot-args is 3, then don't boot kernel, just load iBoot payload
		...
		debug("Booting iBoot in payload mode\n");
		if(boot_iboot() < 0) {
